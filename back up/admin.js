...
async function deleteProduct(id) {
  const p = PRODUCTS.find(pr => pr.id === id);
  if (!confirm(`Delete "${p.name}"? It will be hidden from the store immediately (it's a soft-delete, so order history stays intact).`)) return;
...
function renderCustomersTable() {
  ...
  `).join("") : `<tr><td colspan="4"><div class="admin-empty">No customers yet.</div></td></tr>`;
}
